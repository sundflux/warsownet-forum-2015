 ...
 * 2001,2002,2003 Brent R. Matzelle
 *
 * The Initial Developer of the Original Code is
 * Brent R. Matzelle
 *
 * Portions created by the Initial Developer are Copyright (C) 2001,2002,2003
...
class email_mailer
{
    ...
    */
     ...
     * @param  bool  $draft If true, message is just formed but not sent
     * @param  mixed $msg   Pointer to variable where message header and body are stored.
     * @return bool
     */
    public function Send($draft = false, &$msg = false)
    {
        $msg = array("header"=>"","body"=>"");
        if (!$draft && (count($this->to)+count($this->cc)+count($this->bcc)) < 1) {
            $this->SetError(i18n("Message recipient is missing."));
        ...
        $this->SetMessageType();
        $msg["header"] .= $this->CreateHeader();
        $msg["body"] = $this->CreateBody();
        if ($msg["body"] == "") {
            return false;
        } elseif ($draft) {
            return true;
        ...
        switch ($this->Mailer) {
            case "sendmail":
                return $this->SendmailSend($msg["header"], $msg["body"]);
            case "mail":
                return $this->MailSend($msg["header"], $msg["body"]);
            case "smtp":
                return $this->SmtpSend($msg["header"], $msg["body"]);
            default:
                $this->SetError(i18n("Selected mailer engine is not supported."));
    ...
    public function SendmailSend($header, $body)
    {
        ...
        if (!@$mail = popen($sendmail, "w")) {
            $this->SetError(i18n("Opening Sendmail failed."));
    ...
    public function MailSend($header, $body)
    {
        ...
        if ($this->Sender != "" && strlen(ini_get("safe_mode")) < 1) {
            ...
            // ini_set is quite often disabled on shared hosts
        ...
        if (isset($old_from) && function_exists('ini_set')) {
