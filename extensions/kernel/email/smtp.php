 ...
 *
 * Contributor(s):
 * 2006,2007 Tarmo Alexander Sundström <ta@sundstrom.im>
 * 2007 Joni Halme <jontsa@angelinecms.info>
 */
/**
 * Based on PHPMailer
...
class email_smtp
{
    ...
    */
    public function Connect($host, $port = "",$tval = 30)
    {
        ...
        if (empty($this->smtp_conn)) {
            ...
            //DEBUG("SMTP -> ERROR: ".$this->error["error"].": $errstr ($errno)" . $this->CRLF);
            return false;
        }
        ...
        //DEBUG("SMTP -> FROM SERVER:".$this->CRLF.$announce);
        return true;
    }
    ...
    */
    public function Authenticate($username, $password)
    {
        ...
        if ($code != 334) {
            ...
            //DEBUG("SMTP -> ERROR: ".$this->error["error"].": ".$rply.$this->CRLF);
            return false;
        }
    ...
    public function Connected()
    {
        if (!empty($this->smtp_conn)) {
            ...
            if ($sock_status["eof"]) {
                ...
                //DEBUG("SMTP -> NOTICE:" . $this->CRLF ."EOF caught while checking if connected");
                $this->Close();
    ...
    */
    public function Data($msg_data)
    {
        ...
        $rply = $this->get_lines();
        $code = substr($rply,0,3);
        //DEBUG("SMTP -> FROM SERVER:".$this->CRLF.$rply);
        if ($code != 354) {
            $this->error = array(
    ...
    */
    public function Quit($close_on_error = true)
    {
        ...
        //DEBUG("SMTP -> FROM SERVER:".$this->CRLF.$byemsg);
        $rval = true;
        $e = null;
        ...
        if ($code != 221) {
            ...
            $rval = false;
            //DEBUG("SMTP -> ERROR: ".$e["error"].": ".$byemsg.$this->CRLF);
        }
    ...
    */
    ...
    {
        $this->error = array(
            "error" => "This method, TURN, of the SMTP s not implemented"
        );
        //DEBUG("SMTP -> NOTICE: ".$this->error["error"].$this->CRLF);
        return false;
    }
